    ...
    <section class="ground">
      ...
      <div class="card_ground mt-10 p-8 rounded-lg shadow-lg lg:p-12 lg:col-span-3">
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div>
            <label class="sr-only" for="name"> اسم العملة</label>
            <input v-model="name" class="w-full p-3 text-md rounded-lg text-gray-900 text-gray-900 border-2 border_card placeholder:text-gray-600 " placeholder="اسم العملة ..." type="text" id="name" />
          </div>

          <div>
            <label class="sr-only" for="code">رمز العملة</label>
            <input v-model="code" class="w-full p-3 text-md rounded-lg text-gray-900 border-2 border_card placeholder:text-gray-600 " placeholder="رمز العملة ..." type="text" id="code" />
          </div>
          ...
          <div>
           ...
           <input class="appearance-none rounded-full h-4 w-4 border border-gray-600  checked:bg-emerald-400 checked:border-blue-600 focus:outline-none" type="radio" id="true" value="true" v-model="isActive" />
           <label class="px-4" for="true">مفعلة</label>
           </div>

           <div class="block p-2 mt-3 w-36 h-10 rounded-md " >
           <input class="appearance-none rounded-full h-4 w-4 border border-gray-600  checked:bg-emerald-400 checked:border-blue-600 focus:outline-none" type="radio" id="false" value="false" v-model="isActive" />
           <label class="px-4" for="false">غير مفعلة</label>
...
<path d="M6.00001 15C5.01503 15.0011 4.06427 14.6387 3.32988 13.9823C2.5955 13.3259 2.12913 12.4216 2.02012 11.4427C1.91112 10.4637 2.16714 9.47899 2.73915 8.67711C3.31115 7.87523 4.15891 7.3126 5.12001 7.097C4.84193 5.80009 5.09044 4.44583 5.81086 3.33215C6.53128 2.21846 7.6646 1.43658 8.96151 1.1585C10.2584 0.880417 11.6127 1.12892 12.7264 1.84934C13.84 2.56976 14.6219 3.70309 14.9 5H15C16.24 4.99875 17.4361 5.45828 18.3563 6.28937C19.2765 7.12046 19.8551 8.26383 19.9797 9.4975C20.1043 10.7312 19.7661 11.9671 19.0307 12.9655C18.2953 13.9638 17.2152 14.6533 16 14.9M14 12L11 9M11 9L8.00001 12M11 9V21" stroke="#484848" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          ...
          >
              <span class="font-medium text-gray-900">إضافة العملة </span>
  ...
  <table class="card_ground min-w-full text-sm divide-y divide-gray-200">
    <thead>
      <tr class=" text-lg">
        <th  class="px-4 py-2 font-medium text-right whitespace-nowrap"> اسم العملة</th>
        <th class="px-4 py-2 font-medium text-right whitespace-nowrap"> رمز العملة</th>
        <th class="px-4 py-2 font-medium text-right whitespace-nowrap">حالة العملة</th>
        <th class="px-4 py-2 font-medium text-right whitespace-nowrap">الإعدادات</th>
      </tr>
    </thead>
    <tbody  v-for="currency in currencys" :key="currency.id" class="card_ground divide-y divide-gray-100">
      <tr>
        <td class="px-4 py-2 font-medium  whitespace-nowrap">{{currency.name}}</td>
        <td class="px-4 py-2  whitespace-nowrap">{{currency.code}}</td>
        <td class="px-4 py-2  whitespace-nowrap">{{currency.isActive}}</td>
        <td class="px-4 py-3  whitespace-nowrap ">
         
    <p :class="currency.isActive == '0' ? 'text-white bg-red-500 px-3 py-1.5 rounded text-md font-medium' : 'bg-green-500 text-white px-3 py-1.5 rounded text-md font-medium'">{{ currency.isActive }}</p>

        
            </td>
        <td class="px-4 py-2  whitespace-nowrap">
          <router-link class="inline-flex w-full items-center justify-center bg-white sm:w-auto  p-1 px-5 text-gray-900"
    :to="{
      name: 'EditCurrency',
      params: {id:currency._id}
    }">
...
async handleSubmit(){
  ...
  const response = await axios.post('/api/v1/currency', form, {
    headers : {
